...
class TaskController extends Controller {
  ...
  protected initRouters(): void {
    this.router.get(`${this.path}/:filter/id`, this.list);
  ...
  private async findById(req: Request, res: Response, next: NextFunction): Promise<void|Response> {
    try {
      const { id } = req.params;
  ...
  private async create(req: Request, res: Response, next: NextFunction): Promise<void|Response> {
    try {
  ...
  private async edit(req: Request, res: Response, next: NextFunction): Promise<void|Response> {
    try {
      const { id } = req.params;
      if (ValidationService.validateId(id, next)) return;
  ...
  private async delete(req: Request, res: Response, next: NextFunction): Promise<void|Response> {
    try {
      const { id } = req.params;
      if (ValidationService.validateId(id, next)) return;
